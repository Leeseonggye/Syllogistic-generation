...
def train_iter(wandb, args, model, model_inputs, optimizer, step, num_iter, epoch, device, scheduler = None) :
    b_input_ids = model_inputs["input_ids"].to(device)
    b_masks = model_inputs["attention_mask"].to(device)
    b_labels = model_inputs["labels"].to(device)
    outputs = model(
...
def test(args, model, dataloader, tokenizer, index) :
    ...
    with torch.no_grad():
        ...
        for model_inputs in tqdm(dataloader) :
            ...
            inputs_for_generation = model_inputs["input_for_generation"]
            inputs_for_generation = {key : value.to(device) for key, value in inputs_for_generation.items()}
            generation = model.generate(
                **inputs_for_generation,
                max_length = args.max_len).cpu()[:,256:] # input for generation length == 256

            # calculate bleu, rouge score
            labels = model_inputs["conclusion"].cpu().squeeze(0)
...
def augmented_dataset_merge_kfold(args,target):
    ...
    raw = pd.read_csv(os.path.join(args.data_path, "Avicenna_train.csv"), encoding = 'Windows-1252')
    raw_yes = raw[raw["Syllogistic relation"]=='yes']
    raw_yes['index'] = raw_yes.index
    conc_list = raw_yes['Conclusion'].values
    ...
    if target == 'prem1':
        concat_df = pd.merge(aug_raw[['index','Premise 1', 'Premise 1_f1']],raw_yes[['index','Premise 2','Syllogistic relation','Conclusion']], how = 'inner', on = 'index')

    elif target == 'prem2':
        concat_df = pd.merge(aug_raw[['index','Premise 2', 'Premise 2_f1']],raw_yes[['index','Premise 1','Syllogistic relation','Conclusion']], how = 'inner', on = 'index')
